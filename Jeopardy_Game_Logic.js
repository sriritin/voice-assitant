...
function initRandomCategories() {
    ...
    while (categories.length < 4) {
        if (project.data[i]) {
            const question = project.data[i];
            const category = question.category.toLowerCase()
            if (question.round === 'Jeopardy!' && !categories.includes(category) && isQuestionNew(question)) {
...
async function startRound(p) {
    ...
    if (!questions) {
        ...
        return p.then(roundContext, {state: { price, category, question } });
    }
}

async function endRound(p, isCorrect) {
    const category = p.userData.questionTree[p.state.category];
    const questions = category[p.state.price];
    const question = p.state.question;
    ...
    if (isCorrect) {
        ...
        const prize = parsePrice(question.value);
    ...
    if (playNext) {
        if (questions.length > 1) {
            ...
            category[p.state.price] = questions;
        } else {
            delete category[p.state.price];

            if (!Object.keys(category).length) {
                delete p.userData.questionTree[p.state.category];
                p.state.category = null;
            }
...
const roundEndContext = context(() => {
    follow(
        '(yes|continue|next|let\'s go|go) (the|) (round|game|)',
        p => {
            p.resolve(true);
        }
    );
...
const roundContext = context(() => {
    ...
    follow(
        '(what is|what are|who is|who are|) (the|) $(ANSWER* .+)',
        p => {
            endRound(p, false);
        }
    );
    ...
    follow(
        ...
        p => {
            p.play({
                ...
                value: p.state.question.value,
                total: p.userData.score,
                embeddedPage: true,
                page: 'jeopardy.html',
            });
            p.play(p.state.question.question);
        }
    );
});
